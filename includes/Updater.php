...
final class ArDesignCodFeeUpdater
{
    ...
    {
        if ('plugin_information' !== $action || ! is_object($args) || ! isset($args->slug) || 'ar-design-cod-fee' !== $args->slug) {
            return $result;
        }
        ...
        return (object) array(
            'name' => 'AR Design COD Fee for WooCommerce',
            'slug' => 'ar-design-cod-fee',
            'version' => $version,
            ...
            'sections' => array(
                'description' => __('Samostatný modul pre nastaviteľnú extra dobierku podľa dopravcu vo WooCommerce.', 'ar-design-cod-fee'),
                'changelog' => '' !== $body ? wp_kses_post(nl2br(esc_html($body))) : __('Changelog nie je dostupný.', 'ar-design-cod-fee'),
            ),
        );
    }
        ...
        $response = wp_remote_get(
            ...
            array(
                ...
                'headers' => array(
                    ...
                    'User-Agent' => 'ar-design-cod-fee/' . $this->currentVersion,
                ),
            )
        );
    ...
    private function extractZipAssetUrl(array $releaseData): string
    {
        ...
        foreach ($assets as $asset) {
            ...
            if ('ar-design-cod-fee.zip' === strtolower($name)) {
                return $url;
            }
        }
    ...
    {
        return 'ar_design_cod_fee_release_data_' . md5($this->repositoryFullName);
    }
}
